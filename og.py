...
DF_Vols_w_Financials = cat_a_df_vols.merge(
    MC_per_Product[['DF_Market', 'Location', 'CR_BrandId', "{} MC".format(previous_year),
...
DF_Vols_w_Financials['LYRevenueAvg'] = np.where(DF_Vols_w_Financials["{}Month".format(previous_year)] == 0, 0,
                                                ...
                                                round(DF_Vols_w_Financials["{} Revenue".format(previous_year)] /
                                                      DF_Vols_w_Financials["{}Month".format(previous_year)], 0))

DF_Vols_w_Financials['CYRevenueAvg'] = np.where(DF_Vols_w_Financials["{}Month".format(current_year)] == 0, 0,

                                                round(DF_Vols_w_Financials["{} Revenue".format(current_year)] /
                                                      DF_Vols_w_Financials["{}Month".format(current_year)], 0))

 Calculating Growth

DF_Vols_w_Financials['Growth'] = (DF_Vols_w_Financials['CYRevenueAvg'] - DF_Vols_w_Financials['LYRevenueAvg']) / \
                                 DF_Vols_w_Financials['LYRevenueAvg']
...
pmi_margins['Margin_Volume'] = round((pmi_margins["{} Volume".format(current_year)] * pmi_margins['Margin'].fillna(0)),
                                     0).astype(int)

pmi_margins = pmi_margins.groupby(['DF_Market', 'Location', 'Brand Family']).sum().reset_index()

pmi_margins = pmi_margins[
    ['DF_Market', 'Location', 'Brand Family', "{} Volume".format(current_year), "{} MC".format(current_year),
     'Margin_Volume']]
...
SKU_by_Vols_Margins = DF_Vols_w_Financials.merge(
...
gf = pd.DataFrame(columns=['DF_Market', 'Location', 'TMO', 'Brand Family', 'CR_BrandId', 'SKU', 'Item per Bundle',
                           ...
                           "{} Volume".format(previous_year), "{} Volume".format(current_year),
                           "{}Month".format(previous_year), "{}Month".format(current_year),

                           "{} Revenue".format(previous_year), "{} Revenue".format(current_year),
                           "{} MC".format(previous_year), "{} NOR".format(previous_year),

                           "{} MC".format(current_year), "{} NOR".format(current_year), 'LYRevenueAvg', 'CYRevenueAvg',
                           'Growth', 'Margin'])

for i in DF_Vols_w_Financials.Location.unique():
    DF_Vols = DF_Vols_w_Financials[DF_Vols_w_Financials['Location'] == i]

    green_flag1 = DF_Vols.sort_values("{} Volume".format(current_year), ascending=False).head(
        no_of_sku[no_of_sku['Location'] == i].iloc[0, -2].astype('int'))
...
for i in DF_Vols_w_Financials.Location.unique():
    ...
    red_flag1 = Red_Vols.sort_values("{} Volume".format(current_year), ascending=True).head(
        no_of_sku[no_of_sku['Location'] == i].iloc[0, -1].astype('int'))
...
green_red_list = green_list.merge(red_list, how='outer',
                                  on=['DF_Market', 'Location', 'TMO', 'Brand Family', 'CR_BrandId', 'SKU',
                                      'Item per Bundle']).fillna(0)

green_red_list['Check'] = np.where(green_red_list['Green'] != green_red_list['Red'], 'OK', 'Problem')

green_red_list[(green_red_list['Location'] == 'Zurich') & (green_red_list['Check'] == 'Problem')].sort_values(
    'Location')
...
SELMA_DF_map['Length'] = np.where(
    SELMA_DF_map['Length'].isin(['REGULAR SIZE', 'REGULAR FILTER', 'SHORT SIZE', 'LONG FILTER', 'NAN']), 'KS',
...
duty_free_volumes2 = duty_free_volumes[
    ['DF_Market', 'Location', 'TMO', 'CR_BrandId', 'SKU', "{} Volume".format(current_year)]]

tobacco_range = category_list.merge(
    duty_free_volumes[['DF_Market', 'Location', 'key', "{} Volume".format(current_year)]], how='left',
...
tobacco_range2 = pd.pivot_table(tobacco_range2, index=['DF_Market', 'Location', 'TMO', 'CR_BrandId'],
                                ...
                                aggfunc={"{} Volume".format(current_year): np.sum,
                                         'SKU': np.count_nonzero}).reset_index()
...
Market_Mix = selma_df_products_3.merge(Market_Mix[['DF_Market', 'Location', 'CR_BrandId', 'TMO', 'SKU',
                                                   ...
                                                   "{} Volume".format(current_year)]], how='left',
...
Market_Summary_Comp = Market_Summary_Comp[
    ['DF_Market', 'Location'] + brand_attributes + ['Comp_Seg_SKU', 'Comp Total', 'SoM_Comp']]

Market_Summary_PMI = Market_Summary_PMI[
    ['DF_Market', 'Location'] + brand_attributes + ['PMI_Seg_SKU', 'PMI Total', 'SoM_PMI']]

Market_Summary_PMI[(Market_Summary_PMI['Location'] == 'Zurich') & (Market_Summary_PMI['Flavor'] == 'Regular') & (
            Market_Summary_PMI['Taste'] == 'Full Flavor') &

                   (Market_Summary_PMI['Length'] == 'KS') & (Market_Summary_PMI['Thickness'] == 'STD')]

Market_Summary_Comp[(Market_Summary_Comp['Location'] == 'Zurich') & (Market_Summary_Comp['Flavor'] == 'Regular') & (
            Market_Summary_Comp['Taste'] == 'Full Flavor') &
...
Market_Volume_Table = Market_Summary0.groupby(['DF_Market', 'Location'] + brand_attributes).sum(
    "{} Volume".format(current_year)).reset_index()

Market_Volume_Table[(Market_Volume_Table['Location'] == 'Zurich') & (Market_Volume_Table['Flavor'] == 'Regular') & (
            Market_Volume_Table['Taste'] == 'Full Flavor') &

                    (Market_Volume_Table['Length'] == 'KS') & (Market_Volume_Table['Thickness'] == 'STD')]

Market = Market_Summary_Delta.merge(
    Market_Volume_Table[['DF_Market', 'Location'] + brand_attributes + ["{} Volume".format(current_year)]]
...
cat_b_scores = pd.DataFrame(list_of_tuples, columns=['Location', 'RSQ', 'NumPMI_SKU', 'NumComp_SKU', 'PMI Volume',
                                                     'Comp Volume']).fillna(0)
...
SELMA_dom_map['Length'] = np.where(
    SELMA_dom_map['Length'].isin(['REGULAR SIZE', 'REGULAR FILTER', 'SHORT SIZE', 'LONG FILTER']), 'KS',
...
Market_Summary = Market_Summary[
    ['DF_Market', 'IATA', 'Location', 'TMO'] + brand_attributes + ['SKU', 'Total TMO', 'SoM']]

Market_Summary_PMI = Market_Summary[Market_Summary['TMO'] == 'PMI']

Market_Summary_PMI = Market_Summary_PMI[
    (Market_Summary_PMI['DF_Market'] != 'Spain DP') & (Market_Summary_PMI['DF_Market'] != 'France DP')]

Market_Summary_Comp = Market_Summary[Market_Summary['TMO'] == 'Comp']

Market_Summary_Comp = Market_Summary_Comp[
    (Market_Summary_Comp['DF_Market'] != 'Spain DP') & (Market_Summary_Comp['DF_Market'] != 'France DP')]
...
Market_Summary_Comp = Market_Summary_Comp[
    ['DF_Market', 'IATA', 'Location'] + brand_attributes + ['Comp_Seg_SKU', 'Comp Total', 'SoM_Comp']]

Market_Summary_PMI = Market_Summary_PMI[
    ['DF_Market', 'IATA', 'Location'] + brand_attributes + ['PMI_Seg_SKU', 'PMI Total', 'SoM_PMI']]

Market_Summary_Delta = Market_Summary_Comp.merge(
    Market_Summary_PMI[['DF_Market', 'IATA', 'Location', 'Flavor', 'Taste', 'Thickness',
...
for iata in clusters.IATA.unique():
    ...
    if len(a) == 0:
        ...
        clusterlist['PMI SKU %'] = np.where(clusterlist['PMI Total'] > 0,
                                            clusterlist['PMI SKU'] / clusterlist['PMI Total'], 0)

        clusterlist['Cluster SKU %'] = np.where(clusterlist['Cluster_Total'] > 0,
                                                clusterlist['Cluster Segment'] / clusterlist['Cluster_Total'], 0)
...
Location_Volumes = pd.pivot_table(cat_a_df_vols, index=['Location'],
                                  aggfunc={"{} Volume".format(current_year): np.sum}).reset_index()

PMI_Volumes_0 = cat_a_df_vols[cat_a_df_vols['TMO'] == 'PMI']

PMI_Volumes = pd.pivot_table(PMI_Volumes_0, index=['Location'],
                             aggfunc={"{} Volume".format(current_year): np.sum}).reset_index()
...
final_table_5 = final_table_4.merge(Location_Volumes[['Location', 'Market_Volume']], how='left', on='Location').fillna(
    0)
